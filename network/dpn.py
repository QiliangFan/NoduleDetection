...
class DPN(LightningModule):
    def __init__(self, small=False, num_init_features=64, k_r=96, groups=32,
                 ...
                 num_classes=1, test_time_pool=False):
        ...
        self.classifier = nn.Conv3d(in_chs, num_classes, kernel_size=1, bias=True)
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.acc.update(out.detach().cpu(), nodule.detach().cpu())
        self.log("dice", self.acc.avg, prog_bar=True)
        return batch_idx

    def test_step_end(self):
        with open("metrics.txt", "a") as fp:
            print(self.acc.avg, file=fp)

    def configure_optimizers(self):
        from torch.optim import SGD

        sgd = SGD(self.parameters(), lr=1e-3, momentum=0.2, weight_decay=1e-4)
        return sgd