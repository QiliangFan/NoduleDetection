...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplane, outplane, stride=1):
        ...
        self.conv1 = nn.Conv3d(inplane, outplane, (1, 1, 1), stride=1)
        self.bn1 = nn.BatchNorm3d(outplane)
        self.relu1 = nn.PReLU()

        self.conv2 = nn.Conv3d(outplane, outplane, (3, 3, 3), stride=stride, padding=1)
        self.bn2 = nn.BatchNorm3d(outplane)
        self.relu2 = nn.PReLU()

        self.conv3 = nn.Conv3d(outplane, outplane*self.expansion, (1, 1, 1), stride=1)
        self.bn3 = nn.BatchNorm3d(outplane*self.expansion)
        self.relu3 = nn.PReLU()

        if stride == 2:
            self.shortcut = nn.Conv3d(inplane, outplane*self.expansion, (1, 1, 1), stride=stride)
        else:
            self.shortcut = None
...
class ResBlock(nn.Module):
    def __init__(self, inplane, outplane, stride=1, activateion=nn.PReLU):
    ...
    #             kaiming_normal_(m.weight, a=0.01)
...
class ResNet3D(nn.Module):
    ...
    """
    def __init__(self, block_num=[4, 23]):
