...
class DataModule(LightningDataModule):
    def __init__(self, i: int, fold_num: int, aug_root: str, is_subsample=False):
        """
        i: i-th fold
    ...
    def setup(self, stage):
        ...
        if stage in "fit":
            self.train_pos_data = Data(self.train_pos_files, label=1)
            self.train_neg_data = Data(self.train_neg_files, label=0)
        elif stage in "test":
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        train_data = ConcatDataset([self.train_pos_data, self.train_neg_data])
        # train_data = self.train_pos_data
        train_data = DataLoader(train_data, batch_size=128, pin_memory=True, num_workers=4, shuffle=True)
        return train_data
